...
while True:
    try:
        response = r.xread(streams=last_ids, block=0, count=5)
        if response:
                    ...
                    try:
                        ...
                        if cluster is not None:
                            try:
                                ...
                                X_ordered = np.array([[feature_vector[k] for k in feature_engineering.FEATURE_KEYS]])
                                ...
                                if fallback_model and fallback_scaler:
                                    ...
                                    fallback_vector = np.array([[feature_vector[k] for k in feature_engineering.FEATURE_KEYS]])
                        ...
                        if "FRAUD" in category:
                            ...
                            # Store all features in Redis for consistency
                            redis_mapping = {k: feature_vector[k] for k in feature_engineering.FEATURE_KEYS}
                            r.hset(user_hash_key, mapping=redis_mapping)
                            r.hincrby(user_hash_key, "Transaction_Count", 1)
                        end = time.perf_counter()  # End latency timer
                        latency_ms = (end - start) * 1000
                        print(f"⚡ Transaction latency: {latency_ms:.2f} ms")
                        print(f"\n🚨 Transaction:")
